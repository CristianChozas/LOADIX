...
public class GetCarrierMyLoadsUseCase implements GetCarrierMyLoadsPort {

    private static final int MAX_PAGE_SIZE = 10;
    private static final List<LoadStatus> ACTIVE_CARRIER_STATUSES = List.of(LoadStatus.RESERVED, LoadStatus.IN_TRANSIT);

    private final UserAccountPort userAccountPort;
    private final LoadPort loadPort;

    public GetCarrierMyLoadsUseCase(UserAccountPort userAccountPort, LoadPort loadPort) {
    ...
    public MyLoadsPageResponse execute(String authenticatedEmail, int page, int size, boolean sortAsc) {
        ...
        if (user.role() != UserRole.CARRIER) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Only carrier users can view their loads");
        }

        int sanitizedPage = Math.max(page, 0);
        int sanitizedSize = Math.min(Math.max(size, 1), MAX_PAGE_SIZE);

        var result = loadPort.findByCarrierUserIdAndStatuses(
            user.id(),
            sanitizedPage,
            sanitizedSize,
            sortAsc,
            ACTIVE_CARRIER_STATUSES
        );
